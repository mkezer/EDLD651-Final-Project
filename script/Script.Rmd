...
s_capital_14 %<>% #reorder the variables
  select(fips, county, state_po, bowl, civic, golf, relig, sport, pol, prof, bus, labor, respn, pvote, pop, nccs, assn, sk, year_sc)
```
...
df_wide_16 %<>% # Create democrat margin variable
  mutate(
    dem_margin = (candidatevotes_democrat - candidatevotes_republican) / (candidatevotes_democrat + candidatevotes_republican)
    )
...
model_2016 <- lm(dem_margin ~ bowl + civic + golf + relig + sport + pol + prof + bus + labor + pvote + respn  + pop, # regression model
                 data = df_wide_16)
...
model_2012 <- lm(dem_margin ~ bowl + civic + golf + relig + sport + pol + prof + bus + labor + pvote + respn  + pop, # regression model
                 data = df_wide_12)
...
model_2008 <- lm(dem_margin ~ bowl + civic + golf + relig + sport + pol + prof + bus + labor + pvote + respn  + pop, # regression model
                 data = df_wide_08)
...
model_2000 <- lm(dem_margin ~ bowl + civic + golf + relig + sport + pol + prof + bus + labor + pvote + respn  + pop, # regression model
                 data = df_wide_00)
...
reg_table$Term <- rec(reg_table$Term, #recode terms
                  rec = "(Intercept) = Intercept; bowl = Bowling; civic = Civic; golf = Golf; relig = Religious; sport = Sport;
                         pol = Political; prof = Professional; bus = Business; labor = Labor; pvote = Popular Vote; 
                         respn = Census Rate; pop = Population; else = copy", as.num = F, append = T, suffix = "")
