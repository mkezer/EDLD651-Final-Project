...
#Can combine the command below as part of the piping above.
...
#I recommend combining all commands below a dataset into one piping section. Then can just repeat the same code by simply inserting the new dataset name for each s_capital year (and edit commands where necessary).


# s_capital_97 %>% 
#   s_capital_97[290, 2] <- "District of Columbia, DC" %>%   #add DC's code
#   separate(areaname, c("county","state_po"), sep = ', ') %>%  #create county and state variables
#   select(-fitnes97, -memspt97, -memnec97) %>% #select relevant variables
#   #remove year from variable names
#   colnames[4:18] <- str_sub(colnames(s_capital_97)[4:18], end=-3) %>%
#   mutate(year_sc = 1997) #remove the year from the variable names#create year variable before merging
...
#This is amazing! Merging datasets seems tricky, so it's impressive you were able to pull so many together!
```
...
library(kableExtra) #Should move to the top of the document in a list of all libraries used
...
#Table 2 has some "NA"s. May want to remove NA's listwise, or replace with a word like "other". I also recommend grouping rows by year. I just couldn't figure out how to then remove the list of years within each group!
(table_2 <- table_02 %>% 
  ...
  kable_classic(full_width = FALSE) %>% 
  footnote(footnote_as_chunk = TRUE, 
           general = "N = total number of counties in the US reporting data."))    #group_rows("2000", 1, 4) %>%
    # group_rows("2008", 5, 7) %>% 
    # group_rows("2012", 8, 10) %>% 
    # group_rows("2016", 11, 13))
...
#These maps are also amazing. I hope to learn to do this in the next data visualization course!
```
