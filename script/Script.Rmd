...
- Finally, we merge all election and social capital data in the same dataset (i.e., `df`). In addition, we created another dataset using `pivot_wider()` to have variables for the candidate votes per political party. Then, we removed the intermediate objects (i.e., all data frames except for `df` and `df_wide`).
...
pivot_wider(
    ...
    names_prefix = "candidatevotes_"
  )

df_wide <- df_wide[,-25] #remove NA variable
...
(table_3 <- table_03 %>% 
  kbl(col.names = c("County", "Religious Organizations", "Civic Associations", "Labor Organizations", "Votes Democrat", "Votes Republican","Voter Turnout (%)"),
      digits = 1,
      caption = "<b>Table 3</b><br /> <i>A summary table for the year 2000: Selected social capital and voting behavior in Oregon counties.</i>",
      align =  "c") %>%
  kable_classic(full_width = FALSE))

(table_4 <- table_04 %>% 
  kbl(col.names = c("County", "Religious Organizations", "Civic Associations", "Labor Organizations", "Democrat", "Republican","Voter Turnout (%)"),
      digits = 1,
      caption = "<b>Table 4</b><br /> <i>A summary table for 2016: Selected social capital and voting behavior in Oregon counties.</i>",
      align =  "c") %>%
  kable_classic(full_width = FALSE))
...
# lm(demmargin ~ 1 + bowl + civic + golf + relig + sport + pol + prof + bus + labor + pvote + respn  + pop, data = df_anal_2016) %>%
#  tidy() %>%
# mutate(
#    p.value = scales::pvalue(p.value), 
#    term = c("Democratic margin", "Bowling", "Civic", "Golf", "Religious", 
#             "Sport", "Political", "Professional", "Bus", "Labor", "Voter turnout", "Census response", "Pop")
#  ) %>%
#  knitr::kable(
#    caption = "Coefficient-Level Estimates for a Model Fitted to Estimate Variation in Social Capital.",
...
# Regression table beginnings, still trying to fix with kable
table_5 <- mod1 %>%
...
aggregate_country %>%
  ...
  ggplot(mapping=aes(x=as.character(Year),y=count,color=type_sc)) +
  geom_point() +
...
#spell out types of social capital
