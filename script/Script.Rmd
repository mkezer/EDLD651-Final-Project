...
plot_usmap(data = df2014, regions = "counties", values = "demmargin") +
...
df_usmap_soccap_2014 <- df2 %>%
  ...
  mutate(labor = labor/pop * 1000) %>%
  mutate(nccs = nccs /pop * 1000) %>% 
  select(fips, civic, relig, pol, labor) %>% 
  pivot_longer(cols = c(civic, relig, pol, labor),
              names_to = 
              values_to = soccount


plot_usmap(data = df_usmap_soccap_2014, regions = "counties", values = "civic") +
